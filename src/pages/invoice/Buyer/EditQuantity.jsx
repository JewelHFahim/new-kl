...
import {  } from "../../../redux/feature/supplier/supplierApi";
import { useUpdateBuyerInvoiceMutation } from "../../../redux/feature/buyers/buyerApi";

function EditQuantity({  isOpen, closeModal, item }) {
  const { register, handleSubmit,  } = useForm({quantity: 1});
  const [updateBuyerInvoice] = useUpdateBuyerInvoiceMutation(item.id);


  const onSubmit = (data) => {

    const quantity = { quantity: parseFloat(data.quantity) };
    updateBuyerInvoice({ data: quantity, id: item.id });
      ...
      {isOpen && (
        ...
        >
          <div className="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            >
              <h2 className="text-[15px]">Edit Quantity Buyer</h2>
